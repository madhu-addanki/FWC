...
int main(void)
{
    ...
    while(1)
    {
		...
		if(joystick_data.pwm_value[0] < 0){
			hal_fpga_onion_pwmctrl_enable(a, 1);	
			hal_fpga_onion_pwmctrl_enable(b, 0);	
			hal_fpga_onion_pwmctrl_enable(c, 0);	
			hal_fpga_onion_pwmctrl_enable(d, 1);
			hal_fpga_onion_pwmctrl_enable(e, 1);	
			hal_fpga_onion_pwmctrl_enable(f, 1);	
			hal_fpga_onion_pwmctrl_enable(g, 1);	
		}
		else{	
			hal_fpga_onion_pwmctrl_enable(a, 0);	
			hal_fpga_onion_pwmctrl_enable(b, 0);	
			hal_fpga_onion_pwmctrl_enable(c, 1);	
			hal_fpga_onion_pwmctrl_enable(d, 0);
			hal_fpga_onion_pwmctrl_enable(e, 0);	
			hal_fpga_onion_pwmctrl_enable(f, 1);	
			hal_fpga_onion_pwmctrl_enable(g, 0);	
		}
	}
		...
		if(joystick_data.pwm_value[1] < 0){
			...
			hal_fpga_onion_pwmctrl_enable(d, 1);
			hal_fpga_onion_pwmctrl_enable(e, 1);	
			hal_fpga_onion_pwmctrl_enable(f, 0);	
			hal_fpga_onion_pwmctrl_enable(g, 0);	
		}
	}
	else{
			...
			hal_fpga_onion_pwmctrl_enable(a, 1);	
			hal_fpga_onion_pwmctrl_enable(b, 1);	
			hal_fpga_onion_pwmctrl_enable(c, 1);	
			hal_fpga_onion_pwmctrl_enable(d, 1);
			hal_fpga_onion_pwmctrl_enable(e, 1);	
			hal_fpga_onion_pwmctrl_enable(f, 1);	
